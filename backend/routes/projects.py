...
async def get_projects(current_user: User = Depends(get_current_user)):
    """Get user's projects"""
    owned_projects = await ProjectService.get_projects_by_owner(str(current_user.id))
    collaborated_projects = await ProjectService.get_projects_by_collaborator(str(current_user.id))
...
async def get_project(
    ...
    if project.owner_id != str(current_user.id) and str(current_user.id) not in project.collaborators:
        raise HTTPException(
